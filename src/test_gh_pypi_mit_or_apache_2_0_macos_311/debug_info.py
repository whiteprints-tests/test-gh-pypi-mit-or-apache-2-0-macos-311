...
from test_gh_pypi_mit_or_apache_2_0_macos_311 import __version__
...
def _add_required_distribution(
    required_distribution: list[_DistributionPackage],
    *,
    distribution: str,
    distributions_packages: dict[str, str],
) -> None:
    """Add a distribution and its corresponding package name to the list.

    This function looks up the package name associated with a given
    distribution in a mapping of distributions to package names. If a
    corresponding package name is found, it creates a `_DistributionPackage`
    dictionary containing the distribution metadata and the package name, then
    appends this dictionary to the provided `required_distribution` list.

    Args:
        required_distribution: A list of `_DistributionPackage` dictionaries
            where each dictionary holds metadata for a package and its
            corresponding distribution.
        distribution: The name of the distribution to look up.
    ...
    Example:
        ...
        >>> _add_required_distribution(
        ...     required,
        ...     distribution="sys",
        ...     distributions_packages=distributions_packages
        ... )
        >>>
        >>> len(required) == 0
        True
...
def gather_debug_info() -> DebugInfo:
    ...
    Returns:
        DebugInfo: A dictionary containing:
            ...
            - package_version: The version of the
              `test_gh_pypi_mit_or_apache_2_0_macos_311`
              package.
